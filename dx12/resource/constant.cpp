 ...
 */
void ConstantBufferResourceView::createView(DescriptorHeap& descriptorHeap, ResourceBase* resourceBase) noexcept {
    const auto num    = resourceBase->num();
    const auto stride = resourceBase->stride();

    handle_ = descriptorHeap.allocate(num);

    for (auto i = 0; i < num; ++i) {
        D3D12_CONSTANT_BUFFER_VIEW_DESC cbvDesc = {};
        cbvDesc.BufferLocation                  = resourceBase->resource()->GetGPUVirtualAddress() + i * ALIGN(stride);
